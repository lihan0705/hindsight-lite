...
2. Resolve the launcher from the absolute `<path>` supplied for this skill:
   use the `scripts/open_memorytree.py` beside this `SKILL.md`. Do not construct,
   guess, search for, or substitute another versioned plugin-cache path.
3. Confirm that exact launcher exists. If it does not, stop and tell the user
   to update and reinstall the plugin; do not search older cache directories.
4. Start that launcher in a long-running background terminal:

   ```bash
   python3 <skill-directory>/scripts/open_memorytree.py --bank codex
   ```

5. Read the printed HTTP URL and report it to the user. On WSL this uses the
   distro's private IPv4 so the Windows browser can reach the editor even when
   WSL localhost forwarding is disabled.
6. Keep the terminal running while the user inspects or edits memory. Stop it only when requested.
